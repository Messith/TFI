...
opttime = []
errorbar = []
hlist = np.linspace(0.5,10.5,20)
for h in hlist:
    times_test = np.arange(0.1,10,0.02)
    times_test = list(times_test)
    for i in np.arange(10,45,0.3):
        times_test.append(i)
    for i in np.arange(45,80,0.7):
        times_test.append(i)
    for i in np.arange(80,180,1):
        times_test.append(i)
    ####
    ee = []
    real = 180
...
### plot figure
plt.plot(hlist,opttime,label=r'B=10')
...
####
plt.errorbar(hlist,opterror,errorbar,ecolor='r',label=r'$B=10$')
plt.xlabel(r'$D/J_{max}$')
plt.ylabel(r'$\sqrt{T}\delta\omega_*$')
plt.xscale('log')
plt.yscale('log')
plt.legend()
plt.savefig(str(fieldstren)+'.error_vs_disorder.jpg')
plt.show()



fo = open(str(fieldstren)+"opterror.txt","w")
...
fo = open(str(fieldstren)+"opttime.txt","w")
fo.write("## N=10,D in (0.5,10), B/J=")
fo.write(str(fieldstren))
fo.write("## \n[")
fo.write("[")
...
fo = open(str(fieldstren)+"errorbar.txt","w")
fo.write("## N=10,D in (0.5,10), B/J=")
fo.write(str(fieldstren))
fo.write("## \n[")
fo.write("[")
